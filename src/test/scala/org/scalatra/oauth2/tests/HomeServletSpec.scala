...
class HomeServletSpec extends AkkaSpecification with BaseScalatraSpec {
  ...
  def clearDB = {
    oauth.userProvider.collection foreach { acc =>
      oauth.userProvider.remove(acc, WriteConcern.Safe)
    }
    oauth.authSessions.collection foreach { acc =>
  ...
  def renderAngular(path: String) = get(path) { body must contain("""<div ng-view></div>""") }
  ...
  def is = sequential ^
    "A HomeServlet should" ^
      "when getting /" ^
        ...
        "get the user json for an authenticated json request" ! rootAuthenticatedJson ^ bt ^
      "when getting /login" ^
      ...
      "when posting to /login" ^
        ...
        "when credentials are valid" ^
          ...
          "set a cookie when remember me is ticked" ! loginWith(Map("login" -> "timmy", "password" -> "password", "remmber" -> "true")) ^ bt ^ bt ^
//      "when getting /register" ^
//        "redirect to home if authenticated" ! pending ^
//        "render a register form if not authenticated" ! pending ^ bt ^
//      "when posting to /register" ^
//        "when the data is invalid" ^
//          "return status 422" ! pending ^
//          "response contains error messages" ! pending ^ bt ^
//        "when the data is valid" ^
//          "registers user" ! pending ^
//          "redirects to login for a html request" ! pending ^
...
//          "returns the user json for a json request" ! pending ^ bt ^ bt ^
//      "when getting /forgot" ^
//        "redirects to /home if authenticated" ! pending ^
//        "shows a reset form if unauthenticated" ! pending ^ bt ^
//      "when posting to /forgot" ^
//        "redirects to /home if authenticated" ! pending ^ bt ^
//        "for invalid data" ^
